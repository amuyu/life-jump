...
## 게임 등급 정보 · 주요화면 (스토어 등급이 선행)

콘솔의 "게임 등급 정보" / "자체등급분류 게임물 정보" / "게임 주요화면" 은 **이미 스토어에서 등급을 받은 게임**을 전제한다.
자체등급분류사업자(구글·애플 등)에 등록해야 등급이 생기고, 게임물관리위원회 DB 에 올라간 값을 옮겨 적는 구조.

순서:
1. eye-training 콘솔에서 같은 칸을 어떻게 채웠는지 먼저 확인 — 같은 경로면 그대로.
2. 스토어에 없으면 구글 플레이 등록(TWA/Capacitor 래핑, 개인 계정, IARC 설문 → 전체이용가). 별도 작업.
3. [게임물관리위원회](https://www.grac.or.kr) 검색에 뜨면 등급분류번호·일자·등록자명·사업자명(구글) 을 옮겨 적는다.
   이용등급 전체이용가, 내용정보 전부 해당 없음, 제작업 신고번호는 개인이면 비움, 인감 대신 서명 이미지.
4. "게임 주요화면" 두 쌍: 왼쪽(스토어 화면)과 오른쪽(앱인토스 화면)이 **같은 장면**이어야 한다.
   → 스토어 리스팅 스크린샷도 `store-screenshots.mjs` 의 같은 seed 로 뽑아 두 쪽에 같은 파일을 쓴다.
   - 주요화면 1: `shot-2-play-636x1048.png`
   - 주요화면 2: `shot-1-lobby-636x1048.png`
5. 하단 체크박스 중 "인허가·등록·신고 완료" 는 등급분류가 끝나야 체크.
