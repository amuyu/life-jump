...
같은 웹 빌드가 **두 곳**에서 돈다: 토스 미니앱(WebView, SDK 있음)과 Google Play TWA(크롬이
`life-jump.lazycompany.dev` 를 여는 것, SDK 없음). 빌드를 나누지 않는다 — TWA 가 가리키는 URL 이
미니앱과 같은 소스라, 번들을 나누면 URL 도 나뉘고 두 벌이 조용히 어긋난다.

- **"토스 앱인가?" 가 아니라 "이 기능이 되는가?" 로 묻는다.** 한 번의 검사가 세 가지 실패를 함께
  덮는다: 토스 밖(TWA·브라우저) / 토스 앱 버전 미달 / 미니앱 기능 승인 전.
- **UI 가 없는 기능**(스와이프백 등)은 미지원 시 **조용히 no-op**. `toss/screen.ts` 가 그 형태다.
- **UI 가 딸린 기능**(게임 센터 랭킹 버튼, 리워드 광고 버튼 등)은 미지원 시 **UI 자체를 그리지
  않는다.** 눌러도 안 되는 버튼은 고장이고, 게임 체크리스트의 "모든 UI 가 의도대로 동작" 에 걸린다.
- 능력 확인은 동적 import 라 **비동기**다. 첫 렌더는 "없음" 을 기본값으로 그리고, 확인이 참으로
  끝났을 때만 다시 render 한다. 버튼이 없다가 생기는 건 괜찮지만 있다가 사라지면 이상하다.
- SDK 를 import 하는 곳은 `src/toss/sdk.ts` 하나뿐이다 (위 "SDK 사용 패턴" 절).

| 기능 | Play TWA 에서 | UI |
|---|---|---|
| 스와이프백 차단 (`toss/screen.ts`) | no-op | 없음 — 구현됨 |
| [게임 센터 리더보드](https://developers-apps-in-toss.toss.im/documentation/common/growth/game-center) | 버튼 숨김 | 있음 — 미구현 |
| 인앱 광고 (리워드/전면) | 버튼·보상 경로 숨김 | 있음 — 미구현 |

게임 센터 메모: **별도 사용자 인증이 필요 없다** (토스가 신원을 내부 처리, 응답에 사용자 식별자
없음) — 랭킹만 원한다면 아래 "토스 로그인 + Supabase" 절 전체를 건너뛸 수 있다. 대신 미니앱 정보
승인(1~2 영업일)·토스 앱 5.221.0+ 가 필요하고, 리더보드는 미니앱당 하나, **서버 검증이 없어
클라이언트가 보낸 점수를 그대로 받는다**. 점수 제출은 진입 직후가 아니라 판 종료 후(`finishRun`)에.
